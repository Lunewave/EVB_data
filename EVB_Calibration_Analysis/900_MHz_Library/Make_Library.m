...
for antenna_ind=1:6
    if AZ_steps>1  
        ...
        curAntennaAmp = csvData_Amp_AZ.(fieldName);
        curAntennaPhase = csvData_Phase_AZ.(fieldName);
        ...
        plot(AZ_data, LibMag);
        grid on; hold on;
        plot([AZ_start:-2:AZ_end], flip(curAntennaAmp{freq_indxs_AZ, 4}));
        % plot([-40:5:40], VNAMag)
        % xlim([-40 40])
        xlabel('Azimuth Angle (deg)'); ylabel('Magnitude (dB)');
        title(['Antenna ' int2str(antenna_ind) ' magnitude EL = 0']);
        legend(lib_location, 'Simulation Gain Result 900 MHz', 'VNA Data 900 MHz', 'Location', 'best');
        ...
        simphase =  flip(unwrap(squeeze(curAntennaPhase{freq_indxs_AZ, 4}), 180) - unwrap(squeeze(csvData_Phase_AZ.A1{freq_indxs_AZ, 4}), 180));
    ...
    if EL_steps>1
        ...
        fieldName = ['A' num2str(antenna_ind)];
        curAntennaAmp = csvData_Amp_EL.(fieldName);
        curAntennaPhase = csvData_Phase_EL.(fieldName);
        ...
        plot(EL_data, LibMag);
        grid on; hold on;
        plot([0:2:EL_start], curAntennaAmp{freq_indxs_EL, 4});
        % plot([60:-5:0], VNAMag)
        % xlim([0 60])
        ...
        uicontrol('Style', 'text', ...
                  ...
                  'ForegroundColor', 'k', ...
                  'Tag', 'snr_footer');
        simphase = unwrap(squeeze(curAntennaPhase{freq_indxs_EL, 4}), 180) - unwrap(squeeze(csvData_Phase_EL.A1{freq_indxs_EL, 4}), 180);
