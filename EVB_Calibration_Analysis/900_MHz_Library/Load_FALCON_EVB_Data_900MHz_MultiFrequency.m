function [magnitude, phase, complex_values, frequencies] = Load_FALCON_EVB_Data(path,numAZ, numEL, offset, data_freq)
...
%   complex_values - [6 x numAZ x numEL] matrix of complex values (linear scale)
    ...
    magnitude = zeros(11, 6, numAZ, numEL);
    phase = zeros(11, 6, numAZ, numEL);
    complex_values = zeros(11, 6, numAZ, numEL);
    fc_MHz = 900;
    ...
    for f = 1:length(freqs)
        for i = 1:numEL
            for k = 1:numAZ
                ...
                phase(f, :,k, i)=[median(P_diff1) median(P_diff2) median(P_diff3) median(P_diff4) median(P_diff5) median(P_diff6)];
                magnitude(f, :, k, i) = [median(c1) median(c2) median(c3) median(c4) median(c5) median(c6)];
                for antenna = 1:6
                    complex_values(f, antenna, k, i) = db2mag(magnitude(f, antenna, k, i))*exp(j.*phase(f, antenna, k, i)/180*pi);
                end
            end
        end
    end
