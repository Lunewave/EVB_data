...
%   complex_values - [6 x #of bin files] matrix of complex values
    ...
    phase_median(:, :, :) = squeeze(median(phase, 3, 'omitnan'));
    phase = phase_median;
    magnitude_median(:, :, :) = squeeze(median(magnitude, 3, 'omitnan'));
    magnitude = magnitude_median;
    for antenna = 1:6
        for i = 1:num_files
            for k = 1:num_freqs
                complex_values(antenna, i, k) = db2mag(magnitude(antenna, i, k))*exp(j.*phase(antenna, i, k)/180*pi);
            end
        end
    end
end
