...
for k = 1:numPoints
    ...
    [~, I] = min(abs(antenna_time - data.UTC_seconds(k)));
...
% %% Setup video writer
% videoName = fullfile(path, 'Drone_Flight_Path_XY.mp4');
% v = VideoWriter(videoName, 'MPEG-4');
% v.FrameRate = 10;
% open(v);
% 
...
% grid on;
% margin = 10;
% xlim([min([min(data.x), min(data.y)])-margin, max([max(data.x), max(data.y)])+margin]);
% ylim([min([min(data.x), min(data.y)])-margin, max([max(data.x), max(data.y)])+margin]);
% zlim([min(data.z), max(data.z)+margin]);
% title('3D position with shrinking trail');
% view(0, 90);
% hold on;
% 
% surf(sX, sY, sZ+antenna_height)
% 
% 
% % Initialize drone trail scatter objects (for max trail length)
% h_trail = gobjects(trailLength,1);
% for i = 1:trailLength
%     h_trail(i) = scatter3(nan, nan, nan, baseSize, 'b', 'filled');
% end
% 
% % Current drone marker and vertical line
% h_marker = plot3(nan, nan, nan, 'xk', 'LineWidth', 2, 'MarkerSize', 10);
% h_vline = plot3(nan, nan, nan, ':k');
% 
% % Altitude text
% h_text = text(nan, nan, nan, '', 'FontSize', 10, 'Color', [0.5 0.5 0.5]);
% 
% for k = 1:numPoints
% 
%     %Find Antenna frame closest to drone frame
%     [~, I] = min(abs(antenna_time - data.UTC_seconds(k)))
% 
%     curr_az = deg2rad(AF_ITP_results(I, 1)); curr_el = deg2rad(AF_ITP_results(I, 2));
%     r = sqrt(data.x(k)^2 + data.y(k)^2 + data.z(k)^2)*1.1;
%     x = r*cos(curr_el)*cos(curr_az);
%     y = r*cos(curr_el)*sin(curr_az);
%     z = r*sin(curr_el) + antenna_height;
% 
%     if exist('h_beam', 'var') && isvalid(h_beam)
%         h_beam.XData = [0, x];
%         h_beam.YData = [0, y];
%         h_beam.ZData = [antenna_height, z];
%     else
%         h_beam = plot3([0, x], [0, y], [antenna_height, z], ...
%             'r--', 'LineWidth', 2);
%         scatter3(0, 0, antenna_height, 100, 'k', 'filled');  % Antenna marker
%     end
% 
% 
% 
...
% 
%     % Update trail points
%     for i = 1:length(trailIdx)
%         sizeFactor = baseSize / (length(trailIdx) - i + 1);
%         h_trail(i).XData = data.x(trailIdx(i));
%         h_trail(i).YData = data.y(trailIdx(i));
%         h_trail(i).ZData = data.z(trailIdx(i));
%         h_trail(i).SizeData = sizeFactor;
%     end
%     % Clear unused trail points
%     for i = length(trailIdx)+1:trailLength
%         h_trail(i).XData = nan;
%         h_trail(i).YData = nan;
%         h_trail(i).ZData = nan;
%     end
...
%     h_marker.XData = data.x(k);
%     h_marker.YData = data.y(k);
%     h_marker.ZData = 0;
% 
%     % Update vertical line
%     v_line = linspace(0, data.z(k), 50);
%     h_vline.XData = data.x(k)*ones(1,50);
%     h_vline.YData = data.y(k)*ones(1,50);
%     h_vline.ZData = v_line;
% 
%     % Update altitude text
%     h_text.Position = [data.x(k), data.y(k), data.z(k)];
%     h_text.String = sprintf('Altitude: %.2f m', data.z(k));
% 
