...
save_figs = 1;
ref_lat = 32.26986;       % North is positive
ref_lon = -110.92604;     % West is negative
...
%% Setup video writer
videoName = fullfile(path, 'Drone_Flight_Path_CVS_Only.mp4');
...
for k = startk:endk
    ...
    drawnow;
    pause(0.01);
    sgtitle(['Time = ' num2str(dt(k) - dt(startk)) ' s;    Drone GPS Tick = ' num2str(k) ';'])
...
legend('Drone Ground Truth', 'location', 'best')
subplot(1, 2, 2)
